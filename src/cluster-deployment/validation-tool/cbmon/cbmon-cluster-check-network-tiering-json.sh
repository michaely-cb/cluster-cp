#!/bin/bash
TOOL_HOME=/opt/cbmon/
TIERING_FILE=/opt/cerebras/cluster/net.json
...
NUM_MGMT=`jq -r '.management_nodes[].name' $TIERING_FILE | wc -l`
...
echo "########################"
echo "########################"
for ((i=0; i<$NUM_MGMT; i++))
do
  MGMT_NODE=$(jq -r '.management_nodes['$i'].name' $TIERING_FILE)
