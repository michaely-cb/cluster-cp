	...
	for _, pod := range activePods {
		...
		recordContainerStatus := func(status *v1.ContainerStatus) {
			if status.State.Terminated != nil && status.State.Terminated.ExitCode != 0 {
				...
				log.Warnf("Error pod %s container %s exitCode: %d terminated reason/message: %s %s",
					pod.Name, status.Name, terminated.ExitCode, terminated.Reason, terminated.Message)
				recorder.Eventf(object, v1.EventTypeWarning, terminated.Reason,
					"Error pod %s container %s exitCode: %d terminated reason/message: %s %s",
					pod.Name, status.Name, terminated.ExitCode, terminated.Reason, terminated.Message)
			}
