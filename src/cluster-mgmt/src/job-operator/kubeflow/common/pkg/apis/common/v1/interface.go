...
// We set generate to false because we don't want DeepCopy methods generated for this interface since interfaces are incompatible
...
type ControllerInterface interface {
	...
	// Return the Pod from Informer Cache
	GetPodFromInformerCache(namespace, name string) (metav1.Object, error)

	// Return the Service from Informer Cache
	GetServiceFromInformerCache(namespace, name string) (metav1.Object, error)
	...
	GetEndpoint(namespace, name string) (*v1.Endpoints, error)

	// GetPodDataIP returns pod ip for wsjob data communication, defaults to cilium and can be on multus if enabled
	GetPodDataIP(pod *v1.Pod) ([]string, error)
