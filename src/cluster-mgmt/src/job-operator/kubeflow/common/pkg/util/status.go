...
const (
	...
	// JobCancelled is added in a job when it is cancelled.
	JobCancelledReason = "JobCancelled"
)
...
// IsFailing checks if the job is in failing condition
func IsFailing(status apiv1.JobStatus) bool {
	return IsLastCondition(status, apiv1.JobFailing)
}

// EndingOrEnded checks if the job ended or will end soon.
func EndingOrEnded(status apiv1.JobStatus) bool {
	return IsEnded(status) || IsFailing(status)
}

// IsCancelled checks if the job is cancelled
func IsCancelled(status apiv1.JobStatus) bool {
	return HasCondition(status, apiv1.JobCancelled)
}

// IsEnded checks if the given job is in a terminated state including success/failed
func IsEnded(status apiv1.JobStatus) bool {
	return IsFailed(status) || IsSucceeded(status) || IsCancelled(status)
}

// HasScheduled checks if the given job has scheduled
func HasScheduled(status apiv1.JobStatus) bool {
	return HasCondition(status, apiv1.JobScheduled)
}

// IsRunning checks if the given job is running
func IsRunning(status apiv1.JobStatus) bool {
	return IsLastCondition(status, apiv1.JobRunning)
}

// IsActive checks if the given job is scheduled and not ended
func IsActive(status apiv1.JobStatus) bool {
	return HasScheduled(status) && !IsEnded(status)
}
...
func IsLastCondition(status apiv1.JobStatus, condType apiv1.JobConditionType) bool {
	if len(status.Conditions) == 0 {
		return false
	}
	return status.Conditions[len(status.Conditions)-1].Type == condType
}

func LastCondition(status apiv1.JobStatus) apiv1.JobConditionType {
	if len(status.Conditions) == 0 {
		return ""
	}
...
func setCondition(status *apiv1.JobStatus, condition apiv1.JobCondition) {
	// Do nothing if JobStatus have failed/cancelled condition
	if IsFailed(*status) || IsCancelled(*status) {
		return
	}
...
func filterOutCondition(conditions []apiv1.JobCondition, condType apiv1.JobConditionType) []apiv1.JobCondition {
	...
	for _, c := range conditions {
		...
		if (condType == apiv1.JobFailed || condType == apiv1.JobSucceeded || condType == apiv1.JobCancelled) && c.Type == apiv1.JobRunning {
