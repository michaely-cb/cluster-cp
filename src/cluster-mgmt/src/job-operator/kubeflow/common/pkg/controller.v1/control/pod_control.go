...
const (
	// FailedGetEndpointReason is added in an event and in a job condition
	// when an endpoint for a replica set is failed to get.
	FailedGetEndpointReason = "FailedGetEndpoint"
	// FailedCreateEndpointReason is added in an event and in a job condition
	// when an endpoint for a replica set is failed to be created.
	FailedCreateEndpointReason = "FailedCreateEndpoint"
	// SuccessfulCreateEndpointReason is added in an event when an endpoint for a job
	// is successfully created.
	SuccessfulCreateEndpointReason = "SuccessfulCreateEndpoint"
...
type PodControlInterface interface {
	...
	DeletePod(pod *v1.Pod, object runtime.Object, gracePeriodSeconds *int64) (bool, error)
	// PatchPod patches the pod.
	PatchPod(namespace, name string, data []byte) error
	// CreateEndpoint create endpoint for pod
	CreateEndpoint(namespace string, endpoint *v1.Endpoints) error
}
...
func (r RealPodControl) createPods(nodeName, namespace string, template *v1.PodTemplateSpec, object runtime.Object, controllerRef *metav1.OwnerReference) error {
	...
	if newPod, err := r.KubeClient.CoreV1().Pods(namespace).Create(context.TODO(), pod, metav1.CreateOptions{}); err != nil {
		...
		logger.Debugf("Controller %v created pod %v", accessor.GetName(), newPod.Name)
		r.Recorder.Eventf(object, v1.EventTypeNormal, SuccessfulCreatePodReason, "Created pod: %v", newPod.Name)
	}
	return nil
}

func (r RealPodControl) CreateEndpoint(namespace string, endpoint *v1.Endpoints) error {
	if newEndpoint, err := r.KubeClient.CoreV1().Endpoints(namespace).Create(context.TODO(), endpoint, metav1.CreateOptions{}); err != nil {
		r.Recorder.Eventf(endpoint, v1.EventTypeWarning, FailedCreateEndpointReason, "Error creating: %v", err)
		return err
	} else {
		log.Log.Info("Created endpoint", "name", newEndpoint.Name)
		r.Recorder.Eventf(endpoint, v1.EventTypeNormal, SuccessfulCreateEndpointReason, "Created endpoint: %v", newEndpoint.Name)
	}
	return nil
}

func (r RealPodControl) DeletePod(pod *v1.Pod, object runtime.Object, gracePeriodSeconds *int64) (podTerminating bool, err error) {
	accessor, err := meta.Accessor(object)
	if err != nil {
		return false, fmt.Errorf("object does not have ObjectMeta, %v", err)
	}
	logger := commonutil.LoggerForJob(accessor)
	// if grace period set(as 0), then do delete again
	if (pod.DeletionTimestamp != nil || pod.Annotations["fake-deleted"] == "true") && gracePeriodSeconds == nil {
		logger.Debugf("pod %s/%s is terminating, skip deleting", pod.Namespace, pod.Name)
		return true, nil
	}
	logger.Debugf("Controller %v deleting pod %v/%v", accessor.GetName(), pod.Namespace, pod.Name)
	if gracePeriodSeconds != nil {
		logger.Warnf("pod %s/%s on node %s is stuck at terminating after lock grace period, please check node status",
			pod.Namespace, pod.Name, pod.Spec.NodeName)
	}
	if err := r.KubeClient.CoreV1().Pods(pod.Namespace).Delete(context.TODO(), pod.Name, metav1.DeleteOptions{}); err != nil && !apierrors.IsNotFound(err) {
		r.Recorder.Eventf(object, v1.EventTypeWarning, FailedDeletePodReason, "Error deleting: %v", err)
		return false, fmt.Errorf("unable to delete pods: %v", err)
	} else {
		r.Recorder.Eventf(object, v1.EventTypeNormal, SuccessfulDeletePodReason, "Deleted pod: %v", pod.Name)
	}
	return false, nil
}
...
func (f *FakePodControl) DeletePod(pod *v1.Pod, object runtime.Object, gracePeriodSeconds *int64) (bool, error) {
	f.Lock()
	defer f.Unlock()
...
func (f *FakePodControl) CreateEndpoint(namespace string, endpoint *v1.Endpoints) error {
	return nil
}
