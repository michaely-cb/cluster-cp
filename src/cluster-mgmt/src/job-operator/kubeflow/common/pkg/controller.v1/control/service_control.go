...
type ServiceControlInterface interface {
	...
	DeleteService(service *v1.Service, object runtime.Object) error
}
...
func (r RealServiceControl) createServices(namespace string, service *v1.Service, object runtime.Object, controllerRef *metav1.OwnerReference) error {
	...
	log.Debugf("Controller %v created service %v", accessor.GetName(), newService.Name)
...
// DeleteService deletes the service identified by serviceID.
func (r RealServiceControl) DeleteService(svc *v1.Service, object runtime.Object) error {
	accessor, err := meta.Accessor(object)
	if err != nil {
		return fmt.Errorf("object does not have ObjectMeta, %v", err)
	}
	if svc.DeletionTimestamp != nil {
		log.Infof("service %s/%s is terminating, skip deleting", svc.Namespace, svc.Name)
		return nil
	}
	log.Debugf("Controller %v deleting service %v/%v", accessor.GetName(), svc.Namespace, svc.Name)
	if err := r.KubeClient.CoreV1().Services(svc.Namespace).Delete(context.TODO(), svc.Name, metav1.DeleteOptions{}); err != nil && !apierrors.IsNotFound(err) {
		...
		r.Recorder.Eventf(object, v1.EventTypeNormal, SuccessfulDeleteServiceReason, "Deleted service: %v", svc.Name)
	}
	return nil
}
...
func (f *FakeServiceControl) DeleteService(service *v1.Service, object runtime.Object) error {
	f.Lock()
	defer f.Unlock()
	f.DeleteServiceName = append(f.DeleteServiceName, service.Name)
	if f.Err != nil {
		return f.Err
	}
	return nil
