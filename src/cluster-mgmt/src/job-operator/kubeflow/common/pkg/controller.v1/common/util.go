...
func GenSvcName(jobName string, rtype string, index string) string {
	n := "svc-" + GenGeneralName(jobName, rtype, index)
	return strings.Replace(n, "/", "-", -1)
}
...
// option to fall back to serialized mode in pod/svc reconciling
func (jc *JobController) IsSerializedMode() bool {
	return jc.Config.KubeApiQPS < 50
}

func GetGracePeriodByJobSize(gracePeriod time.Duration, podCount int) time.Duration {
	if podCount < MediumJobPodCount {
		return gracePeriod
	} else if podCount < LargeJobPodCount {
		return gracePeriod * 2
	} else {
		return gracePeriod * 3
	}
}
