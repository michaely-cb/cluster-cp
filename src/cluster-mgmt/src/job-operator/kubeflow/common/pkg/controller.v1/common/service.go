...
func (jc *JobController) ReconcileServices(
	...
	for index, serviceSlice := range serviceSlices {
		if len(serviceSlice) > 1 {
			...
			go func() {
				defer wg.Done()
				err := jc.CreateNewService(job, rtype, spec, id)
				if err != nil {
					commonutil.LoggerForReplica(job, rt).Warnf("create service error: %s-%s, err: %s",
						rtype, id, err)
					returnError = err
				}
			}()
			...
			if index < 0 || index >= replicas {
				...
				go func() {
					defer wg.Done()
					if svcObj, err := jc.Controller.GetServiceFromInformerCache(svc.Namespace, svc.Name); err != nil && !errors.IsNotFound(err) {
						log.Warnf("get svc %s/%s error %v", svc.Namespace, svc.Name, err)
						returnError = err
					} else if svcObj != nil {
						svc = svcObj.(*v1.Service)
						err := jc.ServiceControl.DeleteService(svc, job.(runtime.Object))
						if err != nil {
							log.Warnf("delete svc error: %s/%s, err: %s", svc.Namespace, svc.Name, err)
							returnError = err
						}
					}
				}()
...
func (jc *JobController) CreateNewService(job metav1.Object, rtype apiv1.ReplicaType,
	...
	service.Name = GenSvcName(job.GetName(), rt, index)
