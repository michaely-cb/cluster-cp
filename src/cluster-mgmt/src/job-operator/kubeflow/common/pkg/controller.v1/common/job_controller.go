...
type JobControllerConfiguration struct {
	...
	// Create multus endpoint explicitly if enabled
	EnableMultus bool

	// Grace period for pending pod with certain hanging errors to fail
	PendingPodGracePeriod time.Duration

	// Grace period for lock cleanup waiting for pods terminating
	LockCleanupGracePeriod time.Duration

	// The max time window where client needs to renew the lease in to avoid disconnection
	ClientLeaseDuration time.Duration
}
