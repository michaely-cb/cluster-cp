...
func (jc *JobController) DeletePodsAndServices(runPolicy *apiv1.RunPolicy, job interface{}, pods []*v1.Pod, gracePeriodSeconds *int64) error {
	if len(pods) == 0 {
		return nil
	}
	...
	namespace := ""
	var podsDeleted, podsTerminating int64
	for _, pod := range pods {
		...
		go func() {
			defer wg.Done()
			if podObj, err := jc.Controller.GetPodFromInformerCache(namespace, name); err != nil && !apierrors.IsNotFound(err) {
				log.Warnf("get pod %s/%s error %v", namespace, name, err)
				returnError = err
			} else if podObj != nil {
				pod = podObj.(*v1.Pod)
				terminating, err := jc.PodControl.DeletePod(pod, job.(runtime.Object), gracePeriodSeconds)
				if err != nil {
					log.Warnf("delete pod %s/%s error %v", namespace, name, err)
					returnError = err
				} else if terminating {
			...
			if svcObj, err := jc.Controller.GetServiceFromInformerCache(namespace, GetSvcName(name)); err != nil && !apierrors.IsNotFound(err) {
				log.Warnf("get service %s/%s error %v", namespace, GetSvcName(name), err)
				returnError = err
			} else if svcObj != nil {
				svc := svcObj.(*v1.Service)
				if err = jc.ServiceControl.DeleteService(svc, job.(runtime.Object)); err != nil {
					log.Warnf("delete service %s/%s error %v", namespace, GetSvcName(name), err)
					returnError = err
				}
			}
		}()
	...
	if podsTerminating > 0 || podsDeleted > 0 {
		log.Infof("%d pods are in terminating state, %d pods deleted", podsTerminating, podsDeleted)
	}
	wg.Wait()
	return returnError
}
...
func (jc *JobController) ReconcileJobs(
	...
	log.Infof("Reconciling for job %s, rv %s", metaObject.GetName(), metaObject.GetResourceVersion())
	...
	if commonutil.IsSucceeded(jobStatus) || commonutil.IsFailed(jobStatus) || commonutil.IsCancelled(jobStatus) {
		// Keep error/complete pods/svc if failed, otherwise clean up all
		if !commonutil.IsFailed(jobStatus) &&
			runPolicy.CleanPodPolicy != nil && *runPolicy.CleanPodPolicy != apiv1.CleanPodPolicyNone {
			*runPolicy.CleanPodPolicy = apiv1.CleanPodPolicyAll
		}

		var gracePeriodSeconds *int64
		if jobStatus.CompletionTime.Add(jc.Config.LockCleanupGracePeriod).Before(time.Now()) {
			val := int64(0)
			gracePeriodSeconds = &val
		}
		if err := jc.DeletePodsAndServices(runPolicy, job, pods, gracePeriodSeconds); err != nil {
			return err
		}
	...
	if jobExceedsLimit {
		...
		if err := jc.DeletePodsAndServices(runPolicy, job, pods, nil); err != nil {
			return err
		}
		...
		for rtype, spec := range replicas {
			...
			if err != nil {
				if _, ok = err.(*apiv1.RequeueAfterTTL); !ok {
					log.Warnf("ReconcilePods error %v", err)
					return err
				}
				if requeueTTLError == nil ||
					// requeue with earlier event
					requeueTTLError.(*apiv1.RequeueAfterTTL).TTL > err.(*apiv1.RequeueAfterTTL).TTL {
					requeueTTLError = err
				}
			}
...
func (jc *JobController) CleanupJob(runPolicy *apiv1.RunPolicy, jobStatus apiv1.JobStatus, job interface{}) error {
	...
	// allow lock cleanup grace period first
	lockExpireTime := finishTime.Add(jc.Config.LockCleanupGracePeriod)
	if currentTime.Before(lockExpireTime) {
		key, err := KeyFunc(job)
		if err != nil {
			commonutil.LoggerForJob(metaObject).Warnf("Couldn't get key for job object: %v", err)
			return err
		}
		remaining := lockExpireTime.Add(1 * time.Second).Sub(currentTime)
		jc.WorkQueue.AddAfter(key, remaining)
		return &apiv1.RequeueAfterTTL{TTL: remaining}
	}
