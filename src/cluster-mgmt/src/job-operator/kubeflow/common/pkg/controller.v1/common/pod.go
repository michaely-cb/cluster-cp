...
var (
	...
	MediumJobPodCount = 2500
	LargeJobPodCount  = 5000
)

func init() {
	count, _ := strconv.Atoi(os.Getenv("MEDIUM_JOB_POD_COUNT"))
	if count > 0 {
		MediumJobPodCount = count
		fmt.Printf("MEDIUM_JOB_POD_COUNT: %d", count)
	}
	count, _ = strconv.Atoi(os.Getenv("LARGE_JOB_POD_COUNT"))
	if count > 0 {
		LargeJobPodCount = count
		fmt.Printf("LARGE_JOB_POD_COUNT: %d", count)
	}
}
...
func (jc *JobController) ReconcilePods(
	...
	var requeueTTLError error
	var requeueCount int
	podCount := 0
	for _, r := range replicas {
		podCount += int(*r.Replicas)
	}
	gracePeriod := GetGracePeriodByJobSize(jc.Config.PendingPodGracePeriod, podCount)
	var podsDeleted, podsTerminating, podsCreated, podsPendingCreate int64
	for index, podSlice := range podSlices {
		if len(podSlice) > 1 {
			...
			go func() {
				defer wg.Done()
				if err := jc.createNewPod(job, rType, id, spec, masterRole, replicas); err != nil {
					logger.Warnf("create pod error: %s-%d, err: %s", rt, id, err)
					returnError = err
				} else {
					atomic.AddInt64(&podsCreated, 1)
				}
			}()
			if jc.IsSerializedMode() {
				wg.Wait()
			}
		} else {
			...
			if index < 0 || index >= numReplicas {
				...
				go func() {
					defer wg.Done()
					if podObj, err := jc.Controller.GetPodFromInformerCache(pod.Namespace, pod.Name); err != nil && !errors.IsNotFound(err) {
						log.Warnf("get pod %s/%s error %v", pod.Namespace, pod.Name, err)
						returnError = err
					} else if podObj != nil {
						pod = podObj.(*v1.Pod)
						terminating, err := jc.PodControl.DeletePod(pod, runtimeObject, nil)
						if err != nil {
							logger.Warnf("delete pod error: %s/%s, err: %s", pod.Namespace, pod.Name, err)
							returnError = err
							return
						} else if terminating {
							atomic.AddInt64(&podsTerminating, 1)
						} else {
							atomic.AddInt64(&podsDeleted, 1)
						}
			...
			for _, status := range pod.Status.ContainerStatuses {
				...
				if status.Name == jc.Controller.GetDefaultContainerName() && state.Terminated != nil {
					...
					logger.Infof("Pod: %v.%v exited with code %v, reason %s, phase: %s", pod.Namespace, pod.Name, exitCode, state.Terminated.Reason, pod.Status.Phase)
					if exitCode != 0 {
						jc.Recorder.Eventf(pod, v1.EventTypeWarning, state.Terminated.Reason, "Pod: %v.%v exited with code %v", pod.Namespace, pod.Name, exitCode)
					} else {
			...
			// skip hanging check for every pod which likely has similar pending reason
			// and one requeue event is enough to eventually handle all pending pods
			if requeueCount >= 3 && gracePeriod > 0 {
				logger.Warnf("too many requeue events, skip pending check for rest of rtype: %s", rt)
				gracePeriod = -1
			}
			if err := updateJobReplicaStatuses(jc.KubeClientSet, jobStatus, rType, pod, gracePeriod); err != nil {
				if requeueTTLError == nil ||
					// requeue with earlier event
					requeueTTLError.(*apiv1.RequeueAfterTTL).TTL > err.TTL {
					requeueTTLError = err
				}
				requeueCount++
			}
	...
	if podsTerminating > 0 || podsDeleted > 0 {
		logger.Infof("%d pods are in terminating state, %d pods deleted", podsTerminating, podsDeleted)
	}
	if podsPendingCreate > 0 || podsCreated > 0 {
		logger.Infof("Need to create new pods: %d, %d pods created", podsPendingCreate, podsCreated)
	}
	wg.Wait()
	// retry right away for non-ttl error
	if returnError != nil {
		return returnError
	}
	return requeueTTLError
}

// createNewPod creates a new pod for the given index and type.
func (jc *JobController) createNewPod(job interface{}, rType apiv1.ReplicaType, index int, spec *apiv1.ReplicaSpec, masterRole bool, replicas map[apiv1.ReplicaType]*apiv1.ReplicaSpec) error {
	metaObject, ok := job.(metav1.Object)
	if !ok {
		return fmt.Errorf("job is not a metav1.Object type")
	}
	...
	if err := jc.Controller.SetClusterSpec(job, podTemplate, rType, idxStr); err != nil {
		return err
	}

	// Submit a warning event if the user specifies restart policy for
	// the pod template. We recommend setting it from the replica level.
