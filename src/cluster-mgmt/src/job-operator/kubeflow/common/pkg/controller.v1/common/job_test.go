...
func TestDeletePodsAndServices(T *testing.T) {
	...
	for _, tc := range testcase {
		...
		runningPodService := newService(GetSvcName("runningPod"))
		succeededPodService := newService(GetSvcName("succeededPod"))
		...
		err := mainJobController.DeletePodsAndServices(&runPolicy, job, allPods, nil)
